...
function App() {
  ...
  // 自动流程函数
  const oneClickFlow = async () => {
    try {
      // 直接用常量参数
      interativeRef.current = new (AvatarPlatform as any)({
      ...
      await startRecord();
      message.success('一键流程执行完毕！');
    } catch (e) {
      message.error('一键流程执行出错');
      console.error(e);
    }
  };
    const UnInitSDK = () => {
  ...
  useEffect(() => {
    ...
    avatarConfigForm.setFieldsValue({
      avatar_dispatch: { interactive_mode: 0, content_analysis: 0 },
      stream: {
        protocol: 'xrtc',
        alpha: 0,
    ...
    // 用 requestAnimationFrame 确保页面渲染后再执行自动流程
    requestAnimationFrame(() => {
      setTimeout(() => {
        oneClickFlow();
      }, 200); // 200ms 让表单和 DOM 都渲染好
    });
